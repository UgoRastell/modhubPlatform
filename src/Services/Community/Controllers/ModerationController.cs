    ...
    public class ModerationController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 20)
        {
            try
            {
                ...
                var result = await _reportingService.GetUserReportsAsync(userId, page, pageSize);
        ...
        #region Gestion de la modération (modérateurs)
        ...
        public async Task<ActionResult<ContentReport>> GetReportById(string reportId)
        {
            try
            {
                ...
                // Si ce n'est pas un modérateur, vérifier que l'utilisateur est bien celui qui a signalé
                if (!User.IsInRole("Admin") && !User.IsInRole("Moderator"))
                {
                    var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
                    if (report.ReportedByUserId != userId)
                    {
                        return Forbid("Vous n'avez pas l'autorisation d'accéder à ce signalement");
                    }
                }
            ...
            [FromBody] UpdateReportStatusRequest request)
        {
            try
            {
                ...
                var moderatorName = User.FindFirst(ClaimTypes.Name)?.Value ?? "Modérateur";
